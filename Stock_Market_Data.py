...
# Create Table
def create_table(host_name, username, password, port):
    curr, conn = connect_to_db(host_name, username, password, port)
    try:
...
def connect_to_db(host_name, username, password, port):
    try:
...
def call_api(url, stock_list, headers, params):
    for i, j in enumerate(stock_list):
        response = requests.get(url + j.decode(), headers=headers, params=params)
        ...
        pd_stock = pd_stock.append(pd_stock_inter.tail(1), ignore_index=True)
    ...
    pd_stock['Insert_Update_TS'] = dt.now()
    new_header = [*new_header, *['Stock_Symbol', 'Insert_Update_TS']]
...
def update_db(curr, pd_stock, new_header):
...
def connect_to_db_update_insert(host_name, username, password, port, pd_stock, new_header):
    try:
        curr, conn = connect_to_db(host_name, username, password, port)
        pd_stock_insert = update_db(curr, pd_stock, new_header)
...
# create_table(host_name, username, password, port)

# Extract data using API and push the data to DB.
pd_stock, new_header = call_api(url, stock_list, headers, params)
connect_to_db_update_insert(host_name, username, password, port, pd_stock, new_header)